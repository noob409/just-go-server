...
import {
  ...
  deleteCoEditTripPermission,
} from "../../controllers/api/trip.share.controller.js";
...
TripRouter.post(
  "/",
  upload.single("image"),
  validateBody(["name", "startTime", "endTime"]),
  createTrip
); // Test OK (照片上傳未測試)
...
TripRouter.post(
  "/:id/share",
  validateParams(["id"]),
  validateBody(["linkPermission"]),
  coEditTripPermission
);
TripRouter.delete(
  "/:id/share",
  validateParams(["id"]),
  validateBody(["deleteUserId"]),
  deleteCoEditTripPermission
);

TripRouter.post(
  "/:id/plan/addplace",
  validateBody(["googlePlaceId", "dayId"]),
  validateParams(["id"]),
  placeToPlan
